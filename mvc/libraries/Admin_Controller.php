...
*/
    ...
    private function _permissionManager( $module, $action )
    {
        ...
        if ( $this->session->userdata('usertypeID') == 1 && $this->session->userdata('loginuserID') == 1 ) {
            ...
            if ( isset($userdata['loginuserID']) && !isset($userdata['get_permission']) ) {
                if ( !$this->session->userdata($permission) ) {
                    ...
                    if ( $userdata['usertypeID'] == 3 ) {
                    	if ($user_status == 0){
                    		$permissionArray['master_permission_set']['dashboard'] = 'yes';
                    		$permissionArray['master_permission_set']['invoice'] = 'yes';
                    		$permissionArray['master_permission_set']['invoice_view'] = 'yes';
                    		$permissionArray['master_permission_set']['invoice_edit'] = 'yes';
                    		$permissionArray['master_permission_set']['invoice_delete'] = 'yes';
                    	}else{
                    		$permissionArray['master_permission_set']['take_exam'] = 'yes';
                    	}
                    }
        ...
        //var_dump($sessionPermission);

        //var_dump($this->menu_m->get_order_by_menu([ 'status' => 1 ]));
        ...
        // 	// echo 'Hello';
        // 	// exit();
        // 	if ($permission != 'dashboard' || $permission != 'invoice') {
	...
	public function reportPDF($stylesheet=NULL, $data=NULL, $viewpath= NULL, $mode = 'view', $pagesize = 'a4', $pagetype='portrait', $header='', $footer='') {
		...
		$this->mhtml2pdf->html($html,$header,$footer);
    ...
    public function voucherPDF($stylesheet=NULL, $data=NULL, $viewpath= NULL, $mode = 'view', $pagesize = 'a4', $pagetype='landscape') {
		$designType = 'LTR';
		$this->data['panel_title'] = $this->lang->line('panel_title');
		$html = $this->load->view($viewpath, $this->data, true);
        // var_dump($html);
	...
	public function admissionPDF($stylesheet=NULL, $data=NULL, $viewpath= NULL, $mode = 'view', $pagesize = 'a4', $pagetype='landscape') {
		$designType = 'LTR';
		$this->data['panel_title'] = $this->lang->line('panel_title');
		$html = $this->load->view($viewpath, $this->data, true);
        // var_dump($html);
		...
		if(!empty($stylesheet)) {
	        $stylesheet = file_get_contents(base_url('assets/pdf/'.$designType.'/'.$stylesheet));
// 			exit();
			return $this->mhtml2pdf->create($mode, 'PDf', $stylesheet);
		} else { 
			return $this->mhtml2pdf->create($mode, $this->data['panel_title']);
		}
	}
	...
	public function menuTree($dataset, $sessionPermission) {
    	...
    	foreach ($dataset as $id=>&$node) {
			...
			if($node['link'] == '#' ||  
                ...
                (isset($sessionPermission[$node['link']]) && 

                    $sessionPermission[$node['link']] != "no") ) {
	    		if ($node['parentID'] == 0) {
	    			$tree[$id]=&$node;
	    		} else {
