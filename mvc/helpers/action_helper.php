<?php
    ...
    function btn_view( $uri, $name ,$target="self")
    {
        if ( visibleButton($uri) ) {
            if ($target=='_blank') {
    ...
    function display_menu( $nodes, &$menu )
    {
        ...
        if ($_SESSION['usertype'] == 'Student') {
            $CI->load->database();
            $sql = "SELECT `active` FROM `student` WHERE `username` = '".$_SESSION['username']."'";
            $q = $CI->db->query($sql);
            if($q->num_rows() > 0){
                $row = $q->row();
        ...
        foreach ( $nodes as $key => $node ) {
             ...
             if ($user_status == 0 && $_SESSION['usertype'] == 'Student') {
                if ( isset($node['child'][53]) ){
                    //Start
